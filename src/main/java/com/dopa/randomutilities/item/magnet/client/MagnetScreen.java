...
import com.dopa.randomutilities.core.filter.client.GhostFilterClicks;
...
import com.dopa.randomutilities.core.gui.panel.RgbCosmeticPanel;
import com.dopa.randomutilities.core.gui.panel.ScrollingInfoPanel;
import com.dopa.randomutilities.core.gui.widget.FilterModeButton;
...
import com.dopa.randomutilities.item.magnet.client.panel.MagnetConfigPanel;
...
public class MagnetScreen extends AbstractContainerScreen<MagnetMenu> {
    ...
    @Nullable
    private RgbCosmeticPanel cosmeticPanel;
    @Nullable
    ...
    protected void init() {
        ...
        cosmeticPanel = new RgbCosmeticPanel(
                PanelAnchor.LEFT_LOW,
                () -> menu.getOverlayColor(),
                color -> ClientPacketDistributor.sendToServer(new MagnetSettingPayload(
                        MagnetSettingPayload.KIND_COLOR, color)),
                this::addOverlayWidget,
                "gui.dopasrandomutilities.item_magnet.cosmetic.swatch.tooltip",
                new RgbCosmeticPanel.Toggle(
                        "gui.dopasrandomutilities.item_magnet.particles",
                        "gui.dopasrandomutilities.item_magnet.particles.tooltip",
                        "gui.dopasrandomutilities.item_magnet.particles.enabled",
                        "gui.dopasrandomutilities.item_magnet.particles.disabled",
                        () -> menu.isParticlesEnabled(),
                        () -> ClientPacketDistributor.sendToServer(new MagnetSettingPayload(
                                MagnetSettingPayload.KIND_PARTICLES,
                                menu.isParticlesEnabled() ? 0 : 1))
                )
        );
    ...
    public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        UpgradeSlotTooltips.applyHover(
                ...
                menu.upgrades(),
                List.of(),
                stackUpgradeTotalOverride(hoveredSlot)
        );
    ...
    @Nullable
    private Component stackUpgradeTotalOverride(@Nullable Slot hoveredSlot) {
        if (hoveredSlot == null || !hoveredSlot.hasItem()) {
            return null;
        }
        if (hoveredSlot.getItem().getItem() instanceof MachineUpgradeItem upgrade
                && upgrade.kind() == MachineUpgradeItem.Kind.STACK) {
            int stacks = menu.upgrades().stackCount();
            // Same formula as transfer/collector: default magnet batch + stack bonus.
            return UpgradeSlotTooltips.stackPickupTotalLine(
                    MagnetConfig.basePickupBatch(),
                    stacks
            );
        }
        return null;

package com.dopa.randomutilities.magnet.client.panel;
...
/** Overlay tint RGB controls and particle toggle for the item magnet. */
public final class MagnetCosmeticPanel extends AttachedPanel {
    ...
    public MagnetCosmeticPanel(MagnetScreen screen) {
        super(
    ...
    private void commitPendingColor() {
        ...
        if (!suppressCommit && pendingColor != screen.getMenu().getOverlayColor()) {
            ClientPacketDistributor.sendToServer(new MagnetSettingPayload(
                    MagnetSettingPayload.KIND_COLOR,
                    pendingColor
            ));
        }
    }
    ...
    protected void renderContents(GuiGraphicsExtractor graphics, Font font, int bodyX, int bodyY,
        ...
        if (isMouseOverRect(mouseX, mouseY, swatchX, swatchY, 10, SWATCH_H)) {
            graphics.setTooltipForNextFrame(font,
                    Component.translatable("gui.dopasrandomutilities.item_magnet.cosmetic.swatch.tooltip"),
                    mouseX, mouseY);
        }

        drawLabel(graphics, font, Component.translatable("gui.dopasrandomutilities.item_magnet.particles"),

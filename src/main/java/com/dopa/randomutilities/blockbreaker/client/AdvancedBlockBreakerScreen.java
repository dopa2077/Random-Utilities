...
public class AdvancedBlockBreakerScreen extends AbstractContainerScreen<AdvancedBlockBreakerMenu>
    ...
    public AdvancedBlockBreakerScreen(AdvancedBlockBreakerMenu menu, Inventory inventory, Component title) {
        super(menu, inventory, title, 176, AdvancedBlockBreakerMenu.IMAGE_HEIGHT);
    ...
    public void sendVolumeSetting(byte kind, int value) {
        ClientPacketDistributor.sendToServer(new AdvancedBlockBreakerSettingPayload(kind, value));
    }
    ...
    protected void init() {
        ...
        panelHost.add(new ScrollingInfoPanel(
                "gui.dopasrandomutilities.panel.info.advanced_block_breaker.intro"
        ));
        ...
        modeButton = new FilterModeButton(
                leftPos + AdvancedBlockBreakerMenu.ICON_X,
                topPos + AdvancedBlockBreakerMenu.FILTER_SLOT_Y,
                filterModeTooltip(),
    ...
    public void extractBackground(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        FilterRow.blit(
                graphics,
                xo + AdvancedBlockBreakerMenu.ICON_X,
                yo + AdvancedBlockBreakerMenu.FILTER_SLOT_Y,
                AdvancedBlockBreakerMenu.FILTER_SLOT_COUNT,
                i -> menu.slots.get(menu.filterSlotStart() + i).hasItem()
        );
        MachineEnergyBar.render(
                graphics,
                xo + AdvancedBlockBreakerMenu.ENERGY_BAR_X,
                yo + AdvancedBlockBreakerMenu.ENERGY_BAR_Y,
                AdvancedBlockBreakerMenu.ENERGY_BAR_W,
                AdvancedBlockBreakerMenu.ENERGY_BAR_H,
                menu.energyStored(),
                menu.energyCapacity()
        );
    }
    ...
    public void extractContents(GuiGraphicsExtractor graphics, int mouseX, int mouseY, float partialTick) {
        ...
        if (hoveredSlot != null && menu.isPickaxeSlotIndex(hoveredSlot.index) && !hoveredSlot.hasItem()) {
            graphics.setTooltipForNextFrame(
                    font,
                    Component.translatable("gui.dopasrandomutilities.advanced_block_breaker.pickaxe.optional"),
                    mouseX,
                    mouseY
            );
            return;
        ...
        if (MachineEnergyBar.isHover(
                mouseX,
                mouseY,
                leftPos + AdvancedBlockBreakerMenu.ENERGY_BAR_X,
                topPos + AdvancedBlockBreakerMenu.ENERGY_BAR_Y,
                AdvancedBlockBreakerMenu.ENERGY_BAR_W,
                AdvancedBlockBreakerMenu.ENERGY_BAR_H
        )) {
    ...
    private void renderGhostSlotTints(GuiGraphicsExtractor graphics) {
        for (int i = 0; i < AdvancedBlockBreakerMenu.FILTER_SLOT_COUNT; i++) {
